...
public class DynamaxListener {
    ...
    public void onDynamax(DynamaxEvent.BattleEvolve event) {
        if (Tournament.instance() != null && Tournament.instance().state == TournamentStates.ACTIVE) {
            ...
            if (disallowedMechanicRule != null && disallowedMechanicRule.isDynamaxDisallowed() || (new HasSpecFlagRequirement("rental")).isDataMatch(event.pw.pokemon) && (new HasSpecFlagRequirement("!candynamax")).isDataMatch(event.pw.pokemon)) {
